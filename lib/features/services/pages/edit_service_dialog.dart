...
class EditServiceDialog extends StatefulWidget {
  ...
  const EditServiceDialog({super.key, required this.service, required this.departments});
...
class _EditServiceDialogState extends State<EditServiceDialog> {
        ...
        actions: [
          ...
          ElevatedButton(
            onPressed: () async {
              ...
              if (duration == null || departmentId == null) {
                showCustomToast(context, "يجب اختيار المدة والقسم", success: false);
                return;
              }
            ...
            child:  Text("حفظ",style: TextStyle(color:  AppColors.offWhite),),
          ),
        ],
      ),
    );
  ...
  Widget _buildImagePreview() {
    ...
    if (widget.service.imageUrl != null && widget.service.imageUrl!.startsWith('http')) {
      return Image.network(widget.service.imageUrl!, fit: BoxFit.cover);
    }
